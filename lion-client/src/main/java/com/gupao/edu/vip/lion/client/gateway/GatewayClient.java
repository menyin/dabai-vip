...
public class GatewayClient extends NettyTCPClient {
    ...
    protected void initPipeline(ChannelPipeline pipeline) {
        ...
        if (trafficShapingHandler != null) {
            pipeline.addFirst(trafficShapingHandler);//cny_note 添加全局流量整形
        }
    }
    ...
    protected void initOptions(Bootstrap b) {
        super.initOptions(b);
        if (snd_buf.gateway_client > 0) b.option(ChannelOption.SO_SNDBUF, snd_buf.gateway_client);//cny_note 设置发送缓冲区大小
        if (rcv_buf.gateway_client > 0) b.option(ChannelOption.SO_RCVBUF, rcv_buf.gateway_client);//cny_note 设置接收缓冲区大小
    }
