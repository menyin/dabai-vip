...
public final class GatewayClientChannelHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        receiver.onReceive(packet, connectionManager.get(ctx.channel()));//cny_note 在channelActive已经保存了connection
    }

    @Override
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        Connection connection = connectionManager.get(ctx.channel());
        Logs.CONN.error("client caught ex, conn={}", connection); //？？为什么在此没有记录异常信息cause，而下面一条代码却有记录
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        connectionManager.add(connection);//cny_note 放到连接管理中
