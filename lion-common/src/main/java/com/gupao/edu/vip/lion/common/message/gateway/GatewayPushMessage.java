...
public final class GatewayPushMessage extends ByteBufMessage implements IPushMessage {
    ...
    public static GatewayPushMessage build(Connection connection) {
        Packet packet = PacketFactory.get(GATEWAY_PUSH); //cny_note 这里得到的应该是UDPPacket实例
        packet.sessionId = genSessionId();//？？什么情况下会设置这个sessionId ！所有由客户端（包括商家和骑手）发起的消息都要有这个设置，它是一个消息的唯一标识。
    ...
    /* cny_note IPushMessage 接口方法 begin  */
    @Override
    ...
    /* cny_note IPushMessage 接口方法 end  */
