...
/**
 *
 * cny_note 该类是对RemoteRouterManager的缓存包装
 */
public final class CachedRemoteRouterManager extends RemoteRouterManager {
    private final Cache<String, Set<RemoteRouter>> cache = CacheBuilder //cny_note 使用guava的缓存功能
            .newBuilder()
    ...
    public Set<RemoteRouter> lookupAll(String userId) {
        Set<RemoteRouter> cached = cache.getIfPresent(userId); //cny_note 本地缓存一份路由
