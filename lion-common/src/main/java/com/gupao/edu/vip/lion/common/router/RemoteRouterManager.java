...
public class RemoteRouterManager extends EventConsumer implements RouterManager<RemoteRouter> {
    ...
    public RemoteRouter register(String userId, RemoteRouter router) {
        String key = CacheKeys.getUserRouteKey(userId);//cny_note 骑手用户的用户id
        String field = Integer.toString(router.getRouteValue().getClientType()); //cny_note 骑手用户的设备类型
        ClientLocation old = cacheManager.hget(key, field, ClientLocation.class);
        cacheManager.hset(key, field, router.getRouteValue().toJson()); //？？这两句代码是不是写反了 ！没有写反，只是返回了旧的路由
    ...
    /**
     * 目前的实现方式是非原子操作(get:set)，可能会有并发问题，虽然概率很低 cny_note 可以用lua脚本保证原子性
     * 后续考虑采用lua脚本，实现原子操作
     *
     * @param userId     用户ID
