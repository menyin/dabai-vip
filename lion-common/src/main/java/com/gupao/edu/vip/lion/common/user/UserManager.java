...
public final class UserManager {
    ...
    private final String onlineUserListKey = CacheKeys.getOnlineUserListKey(ConfigTools.getPublicIp());//cny_note 每个分布式服务节点的在线列表是单独存在一个redis key里的
        ...
        if (remoteRouters != null) {
            for (RemoteRouter remoteRouter : remoteRouters) {
                ...
                if (clientType == -1 || location.getClientType() == clientType) {
                    ...
                    mqClient.publish(Constants.getKickChannel(location.getHostAndPort()), message);// cny_note 在RouterChangeListener里订阅了该消息
