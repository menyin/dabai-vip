...
public final class RouterCenter extends BaseService {
    ...
    protected void doStart(Listener listener) throws Throwable {
        ...
        routerChangeListener = new RouterChangeListener(lionServer);//cny_note 实例化RouterChangeListener就启动了一个redis的mqClient订阅监听，在这个实例里可以监听到踢人消息
        userEventConsumer = new UserEventConsumer(remoteRouterManager);//cny_note UserEventConsumer初始化就注册了EventBus事件监听
    ...
    /**
     * 注册用户和链接
     * cny_note ？？这里面在注册路由出现异常并没有return false,是不是有逻辑问题
     * @param userId
     * @param connection
     * @return
     */
    public boolean register(String userId, Connection connection) {
        ...
        } catch (Exception e) {//？？这里是否应该返回false
