...
/**
 * cny_note 此类通过启动一个mqClient来监听消息，监听到消息后再去操作路由变化
 * 并且还通过EventBus来监听路由的变化
 */
public final class RouterChangeListener extends EventConsumer implements MQMessageReceiver {
    ...
    public RouterChangeListener(LionServer lionServer) {
        ...
        if (!udpGateway) {
            ...
            mqClient.subscribe(getKickChannel(), this); //cny_note 此处订阅了踢人的消息，
        }
    }
    ...
    /**
     * ？？为什么这里监听路由变化时，是发送踢人消息
     * ！！因为此时event接收的其实是旧的路由信息，从RouterCenter#register方法中可以看出，EventBus.post(老的路由)
     * @param event
     */
    @Subscribe
    ...
    private void sendKickUserMessage2Client(final String userId, final LocalRouter router) {
        ...
        message.send(future -> {
            if (future.isSuccess()) {//cny_note ？？为什么踢人只是发踢人消息到客户端，而且踢人消息发送成功后也没有直接把connection关闭，没有删除路由信息。
    ...
    private void sendKickUserMessage2MQ(String userId, RemoteRouter remoteRouter) {
        ...
        if (udpGateway) {//？？这里发送的网关踢人消息 是发到骑手客户端上，难道让骑手客户端自己断开连接？
            ...
            MQKickRemoteMsg message = new MQKickRemoteMsg()
                    ...
                    .setTargetServer(location.getHost())
                    .setTargetPort(location.getPort());
            mqClient.publish(Constants.getKickChannel(location.getHostAndPort()), message);//cny_note 这个消息只会被指定的connectionServer节点的订阅到
        }
    }
    ...
    public void onReceiveKickRemoteMsg(KickRemoteMsg msg) {
        ...
        if (localRouter != null) {
            ...
            if (localRouter.getRouteValue().getId().equals(msg.getConnId())) {//二次校验，防止误杀
                //cny_note ？？为什么踢人只是发踢人消息到客户端，而且踢人消息发送成功后也没有直接把connection关闭，没有删除路由信息。
    ...
    /**
     * cny_note 监听RedisMq广播的踢人消息
     * @param topic
     * @param message
     */
    @Override
    public void receive(String topic, Object message) {
        if (getKickChannel().equals(topic)) {//cny_note 实际上这个应该是不用判断，因为在订阅的时候，当前分布式节点只会订阅当前节点相关的消息
