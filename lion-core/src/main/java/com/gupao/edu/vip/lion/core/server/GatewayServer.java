...
/**
 * cny_note 网关服务端，应该是每个连接服务器节点都会有自己的一个网关服务端。
 *          业务系统通过PushClient->gatewayClient连接到 网关服务端GatewayServer
 *          其中NettyTCPClient是gatewayClient实现
 *          如果是这样，则远程路由保存的应该是骑手客户端对应连接服务器节点的网关服务的ip+port信息
 */
public final class GatewayServer extends NettyTCPServer {
    ...
    public GatewayServer(LionServer lionServer) {
        ...
        this.connectionManager = new ServerConnectionManager(false);//cny_note 猜测网关服务对接的是商家客户端，所以连接的数量应该没有那么多，所以不做心跳
    ...
    protected void initOptions(ServerBootstrap b) {
         ...
         * 这个时候如果还是不断地写数据，这个buffer就会不断地增长，最后就有可能出问题了(我们的情况是开始吃swap（虚拟内存），最后进程被linux killer干掉了)。
