...
public final class FastConnectHandler extends BaseMessageHandler<FastConnectMessage> {
    ...
    public void handle(FastConnectMessage message) {
        ...
        } else if (!session.context.deviceId.equals(message.deviceId)) {//cny_note 不同设备不能交叉发送快速链接消息
        ...
        } else {//？？这里为什么没有更新reusableSession的过期时间，按逻辑是要更新下过期时间的
            //3.校验成功，重新计算心跳，完成快速重连 ？？为什么要重新计算心跳
