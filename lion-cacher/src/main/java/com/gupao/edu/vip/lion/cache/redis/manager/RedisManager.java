...
public final class RedisManager implements CacheManager {
    ...
    private final RedisConnectionFactory factory = new RedisConnectionFactory(); //cny_note 这个类代码是org.springframework.data.redis.connection.jedis.JedisConnectionFactory改造的
    ...
    public void init() {
        ...
        if (CC.lion.redis.isSentinel()) {
            factory.setSentinelMaster(CC.lion.redis.sentinelMaster);//cny_note 哨兵监控的master名称，在sentinel.conf里配置的
        }
        factory.init();
        test();
    ...
    private <R> R call(Function<JedisCommands, R> function, R d) {
        if (factory.isCluster()) {
            ...
            try (Jedis jedis = factory.getJedisConnection()) { //cny_note  在jdk1.7+后try-with-resources写法会自动掉用close，
    ...
    public void subscribe(final JedisPubSub pubsub, final String channel) {
        Utils.newThread(channel, //cny_note 因为在订阅时，jedis#subscribe会阻塞当前线程，因此必须另起一个线程执行
                () -> call(jedis -> {
                    if (jedis instanceof MultiKeyCommands) {
    ...
    public void test() {
        if (factory.isCluster()) {
            JedisCluster cluster = factory.getClusterConnection(); //cny_note cluster模式下自带连接池，所以cluster可以是一个单例
            if (cluster == null) throw new RuntimeException("init redis cluster error.");
        } else {
            Jedis jedis = factory.getJedisConnection();//cny_note 从连接池里获取一个Jedis实例，用完需要关闭
