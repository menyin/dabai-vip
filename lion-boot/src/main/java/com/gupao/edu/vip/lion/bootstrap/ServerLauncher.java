...
public final class ServerLauncher {
    ...
    public void init() {
        ...
        serverEventListener.init(lionServer);//cny_node 相当于一个中间件，这个中间件由ServerEventListenerFactory按照自己的SPI规则生产。这里调用了中间件的init方法
        //chain逻辑是每个BootJob会持有一个自己的引用，算是装饰器模式
        chain.boot()
                //cny_note所有启动类xxxBoot都做了对实际的模块做了包装和统一的抽象
