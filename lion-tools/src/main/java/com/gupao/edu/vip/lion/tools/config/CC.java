...
public interface CC {
    ...
    interface lion {
        ...
        interface core {
            ...
            long compress_threshold = cfg.getBytes("compress-threshold"); //配置形如“128M”、“128K”
        ...
        interface redis {
            ...
            List<RedisNode> nodes = cfg.getList("nodes")
                    .stream()//第一纬度数组
                    .map(v -> RedisNode.from(v.unwrapped().toString())) //cny_note unwrapped()返回一个装箱后的引用类型
            ...
            static <T> T getPoolConfig(Class<T> clazz) {
                return ConfigBeanImpl.createInternal(cfg.getObject("config").toConfig(), clazz); //？？ConfigBeanImpl应该和beanUtils功能差不多 ？？待研究
            }
        }
