...
/**
 * cny_note 骑手客户端连接服务demo
 * js的demo在lion-core\src\main\resources\index.html
 * <p>
 * 注：对于移动客户端连接netty服务不一定要用netty的Bootstrap客户端吧
 */
public class ConnClientTestMain {
    ...
    private static void testConnClient(int count, String userPrefix, int printDelay, boolean sync) throws Exception {
        Logs.init();
        ConnClientBoot boot = new ConnClientBoot();//cny_note netty客户端
        boot.start().get();
        ...
        if (printDelay > 0) {
            Executors.newSingleThreadScheduledExecutor() //??
                    .scheduleAtFixedRate(
                            () -> System.err.println(ConnClientChannelHandler.STATISTICS) //cny_note 将netty通道的一些记录的统计信息定时输出
        ...
        for (int i = 0; i < count; i++) {
            ...
            ServiceNode node = serverList.get(index);//cny_note 这里相当于负载均衡
