...
public class ZKClient extends BaseService {
    public static final ZKClient I = I(); //cny_note 单例
    private ZKConfig zkConfig;
    private CuratorFramework client;
    private TreeCache cache;
    private Map<String, String> ephemeralNodes = new LinkedHashMap<>(4);
    private Map<String, String> ephemeralSequentialNodes = new LinkedHashMap<>(1);//cny_note 使用LinkedHashMap保证顺序应该是因为这是 临时有序节点
    ...
    @Override
    protected void doStart(Listener listener) throws Throwable {//由父类BaseService调用并传入包装过的listener
        client.start();
        Logs.RSD.info("init zk client waiting for connected...");
        if (!client.blockUntilConnected(1, TimeUnit.MINUTES)) {//cny_note 阻塞知道连接上或者超时抛异常
//        if (!client.blockUntilConnected(10, TimeUnit.MINUTES)) {//cny_note 阻塞知道连接上或者超时抛异常
    ...
    public void init() {
        ...
        if (zkConfig.getDigest() != null) {
            ...
            builder.aclProvider(new ACLProvider() {
                ...
                @Override
                public List<ACL> getAclForPath(final String path) {//cny_note 应该是根据传入的path返回的权限列表
                    return ZooDefs.Ids.CREATOR_ALL_ACL;
                }
            });
        }
    ...
    private void addConnectionStateListener() {
        //cny_note 注意 getConnectionStateListenable()是连接监听，另外还有节点变化监听getListenable()
        //              getUnhandledErrorListenable()错误监听器等监听，这样设置不同监听则addListener()参数就需要传递不同类型实例
        client.getConnectionStateListenable().addListener((cli, newState) -> {
            //cny_note 这个状态应该是客户端断链后，curatorframework进行重连成功后的状态。
            //         此时就需要将之前注册的临时节点再次进行注册，否则这些节点信息就会丢失。而之前通过正常注册的节点信息在注册时都会进行缓存
    ...
    public List<String> getChildrenKeys(final String key) {
        try {
            ...
            result.sort(Comparator.reverseOrder());//？？这里为什么要排序
            return result;
        } catch (Exception ex) {
    ...
    public void registerPersist(final String key, final String value) {
        try {
            if (isExisted(key)) {
                ...
                client.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT).forPath(key, value.getBytes());//cny_note注册的值是字节码
            }
        } catch (Exception ex) {
